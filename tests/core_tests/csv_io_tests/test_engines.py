...
class TestEngines:
    ...
    def test_midfile_comment_line_pyarrow_matches_polars(self, tmp_path):
        ...
        Since the issue #141 fix, duckdb also matches: comment detection is
        disabled in the import queries, so mid-file ``#`` lines are data on
        every engine.
        """
        ...
        for engine in ("pyarrow", "duckdb"):
            df = CSVEngineFactory(str(midfile_comment_csv), engine).create_reader().to_dataframe()
            assert isinstance(df, pl.DataFrame)
            assert df.columns == ["name", "age"]
            assert df.to_dicts() == polars_rows, f"{engine} rows diverge from polars"
        assert {"name": "alice", "age": "30"} in polars_rows
        assert {"name": "bob", "age": "25"} in polars_rows

    def test_leading_comment_block_with_hash_data_rows_all_engines(self, tmp_path):
        """Issue #141: leading ``#`` block + ``#`` data rows + mid-file ``#`` line.

        DuckDB's sniffer auto-detects ``comment='#'`` for this shape and then
        swallows every ``#``-prefixed line including data rows, returning 0
        rows under a fallback header. All engines must yield the polars
        reference result: only the LEADING comment block is skipped; every
        later line — including a mid-file ``#`` line — is data.
        """
        csv_file = tmp_path / "leading_and_hash_data.csv"
        csv_file.write_text("# generated by tool\ncolor,name\n#FF0000,red\n# midfile comment\n#00FF00,green\n")

        polars_reader = CSVEngineFactory(str(csv_file), "polars").create_reader()
        expected_rows = polars_reader.to_dataframe().to_dicts()
        assert {"color": "#FF0000", "name": "red"} in expected_rows
        assert {"color": "#00FF00", "name": "green"} in expected_rows
        assert len(expected_rows) == 3

        for engine in ("duckdb", "pyarrow"):
            reader = CSVEngineFactory(str(csv_file), engine).create_reader()
            df = reader.to_dataframe()
            assert df.columns == ["color", "name"], f"{engine} header mismatch"
            assert df.to_dicts() == expected_rows, f"{engine} rows diverge from polars"
            sample = reader.get_sample()
